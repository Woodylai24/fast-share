...
function registerIpcHandlers(
  ...
  ipcMainInstance.on("send-text", async (event, text, messageId) => {
    // messageId is generated by the renderer so it can track delivery status
    const msgId = messageId || generateMessageId();
    const message = { type: "text", content: text, messageId: msgId };
    ...
    connectedClients.forEach((client) => {
      if (client.ws.readyState === WebSocket.OPEN && client.keyExchangeComplete) {
        ...
        sent = true;
        // Track ACK — if mobile doesn't confirm in 15s, queue + push
        if (client.deviceId) {
          trackPendingAck(msgId, message, client.deviceId);
        }
      } else if (client.ws.readyState === WebSocket.OPEN) {
        // Key exchange not complete — this shouldn't happen with the new
        // _isConnected() guard on mobile, but handle gracefully
